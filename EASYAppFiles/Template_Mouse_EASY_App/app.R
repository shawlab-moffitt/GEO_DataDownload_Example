...
if (!isTruthy(Feature_Selected)) {
  Feature_Selected <- colnames(meta)[2]
}
...
if (Password_Protected) {
  user_base <- tibble::tibble(
    ...
    #password = purrr::map_chr("TeamScience", sodium::password_store),
    permissions = "admin",
    name = "User"
  )
}
    ...
    if (credentials()$user_auth) {
      ...
      observe({
        updateSelectizeInput(session = session, inputId = "heatmapGeneSelec",
                             choices = sort(as.vector(geneList[,1])),selected = CTKgenes, server = T)
      })
      observe({
        meta <- meta_react()
        expr <- expr_react()
